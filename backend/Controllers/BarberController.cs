    ...
    public class BarberController : ControllerBase
    {
        ...
        public async Task<IActionResult> DeactivateBarber(int id)
        {
            try
            {
                ...
                if (!success)
                {
                    return NotFound("Barbeiro não encontrado ou já inativo."); // Retorna 404 se o barbeiro não for encontrado ou já inativo
                }
            ...
            {
                // Captura a exceção e devolve um 400 Bad Request com a mensagem exata do erro
                return BadRequest(new { message = ex.Message });
            }
            catch (Exception)
            {
                // Captura qualquer outro erro inesperado (falha no banco, etc.)
                return StatusCode(500, new { message = "Erro interno ao tentar desativar o barbeiro." });
            }
        }
    }
}