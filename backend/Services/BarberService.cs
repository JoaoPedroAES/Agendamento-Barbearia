...
namespace barbearia.api.Services
{
    public class BarberService : IBarberService
    {
        ...
        {
            // Verifica se o barbeiro tem agendamentos pendentes (Status == 0)
            bool hasPendingAppointments = await _context.Appointments
                .AnyAsync(a => a.BarberId == barberId && a.Status == 0);

            if (hasPendingAppointments)
            {
                // Interrompe o processo e avisa que não pode excluir
                throw new InvalidOperationException("Não é possível desativar este barbeiro. Ele possui agendamentos pendentes no sistema.");
            }
